...
//					Status:       helper.GetStringFromBytes(item, "status"),
//				})
//			}
//		}
//	})
...
//func (b *BitgetSpot) WithDrawHistoryAll(param transfer.GetWithDrawHistoryParams) (records []transfer.WithDrawHistory, err helper.ApiError) {
//	var uri = "/api/v2/spot/wallet/withdrawal-records"
//	reqBody := make(map[string]interface{})
//	reqBody["limit"] = "100"
//	reqBody["startTime"] = fmt.Sprintf("%v", param.Start)
...
//			}
//			// 检查是否还有下一页数据
//			if len(value.GetArray("data")) < 100 {
//				// 没有更多数据，退出循环
//				return
//			}
//			// 获取下一页的 idLessThan 参数，继续分页
//			idLessThan = helper.GetStringFromBytes(value.GetArray("data")[len(value.GetArray("data"))-1], "orderId")
//		})
//		// 如果没有更多数据了，退出
//		if err.HandlerError != nil || idLessThan == "" {
//			break
//		}
//		time.Sleep(time.Second / 5) //限速规则 10次/1s (UID)
//	}
//	return
//}
